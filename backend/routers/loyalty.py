...
from models import FeedbackIn, PushSegmentIn, SocialShareIn
...
TIERS: list = [
    ...
    },
    {
        ...
        "bday_pct": 10,
        "perks": ["1.25× points", "Priority seating", "Birthday −10%"],
    },
    {
        "name": "Gold",
        "min_spend": 15000,
    ...
    },
    {
        ...
        "bday_pct": 25,
        "perks": ["2× points", "Chef's table invite", "VIP stamps 2×", "Birthday −25%"],
    },
]
...
SPIN_PRIZES = [
    ...
    (
        "jackpot_200",
        1,
        {"kind": "voucher", "value": 200, "title": "JACKPOT · HK$200 off"},
    ),
]
...
async def loyalty_summary(member_id: str, user: dict = Depends(get_current_user)):
    ...
    vouchers = await db.vouchers.find(
        {"member_id": member_id, "redeemed_at": None}
    ).to_list(50)
    ...
    if last_spin:
        try:
            can_spin = (
                datetime.now(timezone.utc) - datetime.fromisoformat(last_spin)
            ).total_seconds() >= 86400
        except Exception:
            can_spin = True
    scratch = await db.scratch_tickets.find_one(
        {"member_id": member_id, "claimed_at": None}
    )
    return {
        "member": serialize(m),
        "tier": tier,
...
async def spin_wheel(member_id: str, user: dict = Depends(get_current_user)):
    ...
    if prize["kind"] == "points":
        await db.members.update_one(
            {"_id": _oid(member_id)}, {"$inc": {"points": prize["value"]}}
        )
    elif prize["kind"] == "stamp":
        await db.members.update_one(
            {"_id": _oid(member_id)}, {"$inc": {"stamps": prize["value"]}}
        )
...
async def claim_scratch(ticket_id: str, user: dict = Depends(get_current_user)):
    ...
    if prize["kind"] == "points":
        await db.members.update_one(
            {"_id": _oid(mid)}, {"$inc": {"points": prize["value"]}}
        )
    elif prize["kind"] == "stamp":
        await db.members.update_one(
            {"_id": _oid(mid)}, {"$inc": {"stamps": prize["value"]}}
        )
    elif prize["kind"] == "voucher":
        voucher = await _issue_voucher(
            member_id=mid,
...
async def list_vouchers(
    member_id: Optional[str] = None, user: dict = Depends(get_current_user)
):
    q: dict = {"redeemed_at": None}
    if member_id:
...
async def redeem_voucher(
    vid: str, body: VoucherRedeemIn, user: dict = Depends(get_current_user)
):
    ...
    new_val = (
        prev + v["discount_value"]
        if o.get("discount_type") == "cash"
...
async def on_payment_earn(member_doc: dict, order_doc: dict):
    ...
    if bonus_pts > 0:
        await db.members.update_one(
            {"_id": member_doc["_id"]}, {"$inc": {"points": bonus_pts}}
        )
        awards.append(
            {
                "kind": "points",
                "value": bonus_pts,
                "title": f"+{bonus_pts} bonus pts ({tier['name']} ×{mult})",
            }
        )
    ...
    if new_tier["name"] != tier["name"]:
        awards.append(
            {
                "kind": "tier",
                "title": f"Promoted to {new_tier['name']}!",
                "value": new_tier["name"],
            }
        )
    ...
    if bmonth and datetime.now(timezone.utc).month == int(bmonth):
        ...
        if not already:
            v = await _issue_voucher(
                ...
                discount_value=new_tier["bday_pct"],
                source="birthday",
                ttl_days=30,
            )
            awards.append({"kind": "voucher", "title": v["title"], "voucher": v})
    ...
    if ref and not member_doc.get("referral_awarded"):
        try:
            await db.members.update_one(
                {"_id": member_doc["_id"]},
                {"$set": {"referral_awarded": True}, "$inc": {"points": 200}},
            )
            await db.members.update_one({"_id": _oid(ref)}, {"$inc": {"points": 200}})
            v_new = await _issue_voucher(
                member_id=str(member_doc["_id"]),
                kind="referral",
            ...
            awards.append(
                {"kind": "voucher", "title": v_new["title"], "voucher": v_new}
            )
            awards.append(
                {
                    "kind": "referral_ref",
                    "title": f"Referrer +200pts + voucher (member {ref[:6]}…)",
                    "value": ref,
                    "voucher": v_ref,
                }
            )
...
async def submit_feedback(
    member_id: str, body: FeedbackIn, user: dict = Depends(get_current_user)
):
...
async def social_share(
    member_id: str, body: SocialShareIn, user: dict = Depends(get_current_user)
):
    ...
    if not m:
        raise HTTPException(404, "Member not found")
    cutoff = (datetime.now(timezone.utc) - timedelta(hours=24)).isoformat()
    dupe = await db.social_shares.find_one(
        {"member_id": member_id, "created_at": {"$gte": cutoff}}
    )
    if dupe:
...
async def _match_segment(seg: PushSegmentIn):
    ...
    if seg.tier:
        ...
        if band:
            ...
            upper = (
                TIERS[idx + 1]["min_spend"] if idx + 1 < len(TIERS) else float("inf")
            )
    ...
    if seg.days_inactive:
        ...
        for m in members:
            ...
            if not last_order:
                ...
                last_order.get("closed_at")
                and datetime.fromisoformat(last_order["closed_at"]) < cutoff
            ):
                filtered.append(m)
        members = filtered
    return members
...
async def push_preview(body: PushSegmentIn, user: dict = Depends(get_current_user)):
    ...
    return {
        "count": len(members),
        "sample": [
            {"id": str(m["_id"]), "name": m["name"], "phone": m.get("phone")}
            for m in members[:5]
        ],
    }
...
async def push_send(body: PushSegmentIn, user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "note": (
            "Real Twilio send"
            if twilio_client
            else "MOCKED — set TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_SMS_FROM / TWILIO_WHATSAPP_FROM to enable"
        ),
    }
...
async def loyalty_digest(user: dict = Depends(get_current_user)):
    ...
    vouchers_issued = await db.vouchers.count_documents(
        {"created_at": {"$gte": day_start}}
    )
    vouchers_redeemed = await db.vouchers.count_documents(
        {"redeemed_at": {"$gte": day_start}}
    )
    scratch_claimed = await db.scratch_tickets.count_documents(
        ...
        "top_point_earners": [
            {
                ...
                "name": m.get("name"),
                "points": m.get("points", 0),
                "tier": tier_for(m.get("lifetime_spend", 0))["name"],
            }
    ...
    for L in logs:
        ...
        g = groups.setdefault(
            key,
            {
                ...
                "mocked": 0,
                "failed": 0,
                "sample_body": L.get("body"),
                "first_sent_at": L.get("sent_at"),
            },
        )
        g["sent"] += 1
...
async def send_weekly_digest(user: dict = Depends(get_current_user)):
    ...
    scratch = await db.scratch_tickets.count_documents(
        {"claimed_at": {"$gte": week_ago}}
    )
    # Biggest missed nudges (dismissed status) — top 3 by count
    nudges = await db.upsell_nudges.find(
        {"ts": {"$gte": week_ago}, "status": "dismissed"}
    ).to_list(2000)
    miss_counts: dict = {}
    for n in nudges:
    ...
    for r in recipients:
        await db.push_log.insert_one(
            {
                "member_id": None,
                "member_name": r.get("name") or r.get("email"),
                "phone": r.get("phone"),
                "channel": "whatsapp",
                "voucher_id": None,
                "voucher_code": None,

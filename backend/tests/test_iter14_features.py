...
def test_scheduled_combo_out_of_window_not_applied(admin_sess, products):
    ...
    try:
        ...
        assert (
            "TEST_ITER14_DOTN_OUT" not in applied_names
        ), f"Should NOT be applied out of window: {applied_names}"
        ...
        applied_names = [c["name"] for c in d.get("combos_applied", [])]
        assert (
            "TEST_ITER14_DOTN_OUT" in applied_names
        ), f"Should be applied in window: {applied_names}"
...
def test_auto_close_non_manager_forbidden(bar_sess):
    r = bar_sess.post(
        f"{API}/orders/auto-close", json={"method": "card", "note": "test"}, timeout=15
    )
    ...
    for _ in range(2):
        r = admin_sess.post(
            ...
            json={
                "order_type": "pick_up",
                "lines": [
                    {"product_id": p1["id"], "name": p1["name"], "price": 60, "qty": 1}
                ],
                "discount_type": "none",
                "discount_value": 0,
                "service_charge_pct": 10,
...
def test_merge_orders(admin_sess, products):
    ...
    # Cleanup: pay target
    admin_sess.post(
        f"{API}/orders/{b['id']}/pay",
        json={"method": "cash", "amount": tgt["total"]},
        timeout=15,
    )
...
def test_move_line_same_order_and_cross(admin_sess, products):
    ...
    r = admin_sess.post(
        f"{API}/orders/{src['id']}/move-line",
        json={"line_index": 0, "target_seat": 3},
        timeout=15,
    )
    assert r.status_code == 200, r.text
    ...
    r = admin_sess.post(
        f"{API}/orders/{src['id']}/move-line",
        json={"line_index": 0, "target_order_id": other["id"], "target_seat": 2},
        timeout=15,
    )
    assert r.status_code == 200, r.text

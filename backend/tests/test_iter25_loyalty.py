...
def sess():
    s = requests.Session()
    r = s.post(
        f"{BASE_URL}/api/auth/login", json={"email": ADMIN_EMAIL, "password": ADMIN_PW}
    )
...
def test_paying_order_upgrades_tier_and_adds_stamp(sess, mongo):
    ...
    mongo.members.update_one(
        {"_id": ObjectId(mid)}, {"$set": {"lifetime_spend": 4990.0}}
    )
    ...
    ord_body = {
        ...
        "guests": 1,
        "lines": [
            {"product_id": "p_test", "name": "Test Drink", "price": 50.0, "qty": 1}
        ],
    ...
    r = sess.post(
        ...
        json={"method": "cash", "amount": total, "tip": 0},
    )
    assert r.status_code == 200, r.text
    ...
    assert (
        s["tier"]["name"] == "Silver"
    ), f"expected Silver got {s['tier']['name']} (spend={s['member']['lifetime_spend']})"
    assert s["stamps"] >= 1
...
def test_stamp_card_voucher_issued_at_goal(sess, mongo):
    ...
    r = sess.post(
        f"{BASE_URL}/api/orders/{oid}/pay",
        json={"method": "cash", "amount": 50.0, "tip": 0},
    )
    assert r.status_code == 200
...
def test_voucher_redeem_applies_cash_discount(sess, mongo):
    ...
    ord_body = {
        ...
        "guests": 1,
        "lines": [
            {"product_id": "p_test", "name": "Test Drink", "price": 100.0, "qty": 1}
        ],
        "service_charge_pct": 0.0,
    }
    r = sess.post(f"{BASE_URL}/api/orders", json=ord_body)
    ...
    # Redeem
    r = sess.post(
        f"{BASE_URL}/api/loyalty/vouchers/{vid}/redeem", json={"order_id": oid}
    )
    assert r.status_code == 200, r.text

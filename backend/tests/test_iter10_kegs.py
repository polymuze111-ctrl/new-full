...
def test_keg_auto_decrement_on_pay(s):
    ...
    before_total = sum(
        k["current_ml"] for k in kegs if k["product_id"] == pid and k["status"] == "on"
    )
...
def test_keg_blown_when_hits_zero(s):
    ...
    beer = next(
        (
            p
            for p in prods
            if p.get("kind") == "drink" or "beer" in (p.get("name", "").lower())
        ),
        prods[0],
    )
    body = {
...
def test_full_order_lifecycle(s):
    ...
    payload = {
        ...
        "discount_value": 0,
        "service_charge_pct": 10,
        "notes": "TEST_iter10_life",
    }
    o = s.post(f"{API}/orders", json=payload).json()
    assert o.get("id")
    # fire
    r = s.post(f"{API}/orders/{o['id']}/fire")
    assert r.status_code == 200
    # pay
    r = s.post(
        f"{API}/orders/{o['id']}/pay",
